...
class AIPlayer(Player):
    ...
    def move(self, player_code, board):
        ...
        col, _ = self.minimax(board, 6, True, player_code, -math.inf, math.inf)
    ...
    def goal(self, board, player_code):
        ...
        return self.end_of_game(board, player_code) or self.end_of_game(board, opp_player_code) or len(self.can_play_list(board)) == 0

    def can_play(self, board, col):
        # Ver se a última linha está preenchida
        return board[0][col] == 0

    def can_play_list(self, board):
        # Transforma a resposta do can_play em Array
        loc = []
        for col in range(7):
            if self.can_play(board, col):
                loc.append(col)
        return loc
    ...
    def next_row(self, board, col):
        # Mostra quais as linhas disponíveis
        for r in range(5, -1, -1):
            if board[r][col] == 0:
                return r

    def minimax(self, board, depth, maxminP, player_code, alfa, beta):
        valid_locations = self.can_play_list(board)
        ...
        if depth == 0 or x_goal:
            if x_goal:
                if self.end_of_game(board, player_code):
                    return (None, 99999999)
                elif self.end_of_game(board, opp_player_code):
        ...
        if maxminP:
            ...
            for col in valid_locations:
                ...
                c_board = board.copy()
                self.play(c_board, r, col, player_code)
            ...
            for col in valid_locations:
                r = self.next_row(board, col)
                c_board = board.copy()
                self.play(c_board, r, col, opp_player_code)
                n_score = self.minimax(
    ...
    def end_of_game(self, board, player_code):
        # Horizontal
        for c in range(4):
            for r in range(6):
    ...
    def play(self, board, row, col, player_code):
